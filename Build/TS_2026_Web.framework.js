...
// Begin JS library code
  ...
  function _TSRegisterAudioFocus(objectName, methodName) {
      if (typeof document === 'undefined' || typeof window === 'undefined') return;
      if (window.__tsAudioFocusRegistered) return;
      window.__tsAudioFocusRegistered = true;
  
      var target = UTF8ToString(objectName);
      var method = UTF8ToString(methodName);
  
      var notify = function (hidden) {
        var send = (typeof SendMessage !== 'undefined') ? SendMessage
          : (typeof Module !== 'undefined' && Module.SendMessage) ? Module.SendMessage
          : null;
        if (!send) return;
        try { send(target, method, hidden ? '1' : '0'); } catch (e) { }
      };
  
      var current = function () { return document.hidden === true; };
  
      document.addEventListener('visibilitychange', function () { notify(current()); });
      // Back/forward cache and mobile tab switching do not always fire visibilitychange.
      window.addEventListener('pagehide', function () { notify(true); });
      window.addEventListener('pageshow', function () { notify(current()); });
  
      if (current()) notify(true);
    }
...
var wasmImports = {
  ...
  /** @export */
  TSRegisterAudioFocus: _TSRegisterAudioFocus,
  /** @export */
